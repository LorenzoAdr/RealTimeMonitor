...
from fastapi import FastAPI, Request, WebSocket, WebSocketDisconnect, Query, File, UploadFile, Form, Body
...
def _bootstrap_web_static_from_seed() -> None:
    """Si falta static/index.html, copia un esqueleto mínimo desde web_monitor/static_seed/ (versionado en repo)."""
    seed_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "static_seed")
    ...
    if not os.path.isdir(seed_dir):
        ...
        print(
            "[VarMonitor Web] AVISO: falta static/index.html y no existe static_seed/ junto a app.py.",
            flush=True,
        )
        return
    os.makedirs(STATIC_DIR, exist_ok=True)
    for root, _dirs, files in os.walk(seed_dir):
        ...
        dest_dir = os.path.join(STATIC_DIR, rel) if rel != "." else STATIC_DIR
        os.makedirs(dest_dir, exist_ok=True)
        for name in files:
            shutil.copy2(os.path.join(root, name), os.path.join(dest_dir, name))
    print(
        "[VarMonitor Web] Rellenado web_monitor/static/ desde static_seed/ (sin árbol estático previo).",
        flush=True,
    )


def _merge_static_seed_client_gaps() -> None:
    """Si falta piezas bajo static/js/ (p. ej. entry.mjs), copiar solo lo que falta desde static_seed/ sin sobrescribir."""
    seed_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "static_seed")
    seed_js = os.path.join(seed_dir, "js")
    if not os.path.isdir(seed_js):
        return
    dest_js = os.path.join(STATIC_DIR, "js")
    os.makedirs(dest_js, exist_ok=True)
    copied: list[str] = []
    for name in os.listdir(seed_js):
        src = os.path.join(seed_js, name)
        if not os.path.isfile(src) or not name.endswith(".mjs"):
            continue
        dst = os.path.join(dest_js, name)
    ...
    if copied:
        print(
            "[VarMonitor Web] Completado static/js desde static_seed (solo faltantes, sin sobrescribir): "
            + ", ".join(copied),
            flush=True,
        )
...
async def index():
    ...
    if not os.path.isfile(index_path):
        return HTMLResponse(
            content=(
                "<!DOCTYPE html><html lang=\"es\"><head><meta charset=\"utf-8\"/><title>VarMonitor</title></head>"
                "<body style=\"font-family:system-ui,sans-serif;padding:2rem;max-width:40rem\">"
                "<h1>Cliente web no instalado</h1>"
                "<p>Falta <code>web_monitor/static/index.html</code>. "
                "Copie el árbol <code>static/</code> desde un entorno completo o ejecute el flujo de generación.</p>"
                "</body></html>"
            ),
            status_code=503,
    ...
    try:
        ...
        return JSONResponse({"error": f"No se pudo leer index.html: {e}"}, status_code=500)
    ...
    m = re.search(
        r'<script\s+[^>]*src="/static/js/entry\.mjs(?:\?[^"]*)?"[^>]*>\s*</script>',
        html,
    )
    if m:
        tag = html_main_script_tag()
...
def _save_runtime_config_overrides(updates: dict) -> None:
    ...
    for k, v in clean.items():
        ...
        elif k == "vars_update_full_refresh_sec":
            existing[k] = str(max(0.0, float(v)))
        else:
            existing[k] = str(v)
    output: list[str] = []
    seen: set[str] = set()
...
def _finalize_recording_temp_file(
    temp_path: str | None,
    rows_written: int,
    *,
    recording_format: str | None = None,
) -> tuple[str, str, int]:
    """Cierra grabación: temporal .parquet.part o .tsv.part → record_*.parquet o record_*.tsv según formato elegido."""
    if not temp_path:
        return "", "", 0
    ...
    try:
        ...
        want_parquet_out = fmt_req == "parquet"
        rp = _rp()
        # TSV explícito (sidecar o writer Python): renombrar aunque Parquet Pro esté instalado.
        if low.endswith(".tsv.part") and not want_parquet_out:
        ...
        if rp is None:
            if low.endswith(".tsv.part") and os.path.isfile(temp_path):
                print(
                    "[VarMonitor Web] Parquet solicitado pero el módulo Pro no está disponible; "
                    "se entrega TSV a partir del temporal.",
                    flush=True,
                )
                return _finalize_recording_temp_file(
                    temp_path, rows_written, recording_format="tsv"
                )
            print(
...
@app.post("/api/recordings/save_converted")
async def api_recordings_save_converted(request: Request):
    """Guarda un TSV canónico VarMonitor como {stem}_converted.tsv (o _converted_N si existe)."""
    _ensure_recordings_dir()
    try:
        payload = await request.json()
    except Exception:
    ...
    if not isinstance(basename, str) or not basename.strip():
        return JSONResponse({"error": "basename requerido"}, status_code=400)
    if not isinstance(content, str) or not content.strip():
        return JSONResponse({"error": "contenido vacío"}, status_code=400)
    raw = os.path.basename(basename.strip())
    stem = re.sub(r"[^A-Za-z0-9._-]+", "_", raw).strip("._-")
    if not stem or len(stem) > 120:
        return JSONResponse({"error": "basename inválido"}, status_code=400)
    root = os.path.abspath(RECORDINGS_DIR)
    chosen_fn = None
    chosen_path = None
    for n in range(500):
        fn = f"{stem}_converted.tsv" if n == 0 else f"{stem}_converted_{n + 1}.tsv"
        path_out = os.path.abspath(os.path.join(RECORDINGS_DIR, fn))
        if not path_out.startswith(root + os.sep):
            return JSONResponse({"error": "ruta inválida"}, status_code=400)
        if not os.path.isfile(path_out):
            chosen_fn = fn
            chosen_path = path_out
            break
    if not chosen_fn or not chosen_path:
        return JSONResponse({"error": "no se pudo elegir nombre de fichero libre"}, status_code=500)
    text = content if content.endswith("\n") else content + "\n"

    def _write() -> None:
        with open(chosen_path, "w", encoding="utf-8") as wf:
            wf.write(text)
    ...
    try:
        ...
        return JSONResponse({"error": str(e)}, status_code=500)
    return {"ok": True, "filename": chosen_fn, "path": chosen_path}
...
async def api_admin_runtime_config(request: Request):
    ...
    try:
        ...
        if "vars_update_full_refresh_sec" in payload:
            v = float(payload.get("vars_update_full_refresh_sec"))
            if not math.isfinite(v) or v < 0.0 or v > 3600.0:
                return JSONResponse({"error": "vars_update_full_refresh_sec inválido (0–3600)"}, status_code=400)
            _config["vars_update_full_refresh_sec"] = v
            updates["vars_update_full_refresh_sec"] = v
        ...
        return {
            ...
            "runtime": {
                ...
                "vars_update_full_refresh_sec": float(_config.get("vars_update_full_refresh_sec", 1.0)),
            },
        }
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)
...
def _mavlink_health_scan_paths():
    d = mavlink_health_scan_state_dir()
    return {
        "dir": d,
        "result": os.path.join(d, "mavlink_health_scan_result.json"),
        "request": os.path.join(d, "mavlink_health_scan_request.json"),
        "stop": os.path.join(d, "mavlink_health_scan_stop.json"),
    }


@app.get("/api/mavlink_health_scan/status")
async def api_mavlink_health_scan_status():
    """Estado del último escaneo MAVLink (CoreNexus escribe el JSON en server_state)."""
    p = _mavlink_health_scan_paths()
    out: dict = {"phase": "idle", "has_result": False, "result": None, "request_pending": False}
    try:
        if os.path.isfile(p["request"]):
            out["request_pending"] = True
            out["phase"] = "running"
    except OSError:
        pass
    try:
        if os.path.isfile(p["result"]):
        ...
        pass
    try:
        out["result_mtime_ms"] = int(os.path.getmtime(p["result"]) * 1000) if os.path.isfile(p["result"]) else 0
    except OSError:
        out["result_mtime_ms"] = 0
    return out


@app.post("/api/mavlink_health_scan/start")
async def api_mavlink_health_scan_start(body: dict | None = Body(None)):
    """Escribe mavlink_health_scan_request.json para CoreNexus."""
    p = _mavlink_health_scan_paths()
    os.makedirs(p["dir"], exist_ok=True)
    raw = body if isinstance(body, dict) else {}
    try:
        duration_sec = float(raw.get("duration_sec", raw.get("durationSec", 10)))
    except (TypeError, ValueError):
        duration_sec = 10.0
    duration_sec = max(1.0, min(100.0, duration_sec))
    try:
        hb = int(raw.get("histogram_bins", raw.get("histogramBins", 40)))
    except (TypeError, ValueError):
        hb = 40
    hb = max(8, min(256, hb))
    rid = str(raw.get("request_id") or raw.get("requestId") or "").strip()
    if not rid:
        rid = str(uuid.uuid4())
    payload = {"action": "start", "duration_sec": duration_sec, "request_id": rid, "histogram_bins": hb}
    with open(p["request"], "w", encoding="utf-8") as f:
        json.dump(payload, f)
    return {"ok": True, "request_id": rid, "duration_sec": duration_sec}


@app.post("/api/mavlink_health_scan/stop")
async def api_mavlink_health_scan_stop():
    """Solicita detención del escaneo en curso (mavlink_health_scan_stop.json)."""
    p = _mavlink_health_scan_paths()
    os.makedirs(p["dir"], exist_ok=True)
    try:
        Path(p["stop"]).write_text("{}", encoding="utf-8")
    except OSError:
...
async def websocket_endpoint(ws: WebSocket):
    ...
    # Refresco periódico forzado: mantiene avance en gráficos aunque los valores sean constantes.
    try:
        full_refresh_sec = max(0.0, float(_config.get("vars_update_full_refresh_sec", 1.0)))
    except Exception:
        full_refresh_sec = 1.0
    last_full_vars_update_at = 0.0
    ...
    async def _try_start_alarm_sidecar() -> None:
        ...
        sidecar_poll_hz = int(_config.get("sidecar_poll_hz", 0) or 0)
        ...
        if sidecar_poll_hz > 0:
            args.extend(["--poll-hz", str(sidecar_poll_hz)])
        try:
    ...
    async def _finalize_recording_after_sidecar_exit(alarm_payload: dict | None) -> None:
        ...
        if recording_tmp_fp is not None:
            ...
            try:
                ...
                pass
        recording_tmp_fp = None
        path_saved, fn_saved, size_saved = _finalize_recording_temp_file(
            recording_tmp_path, recording_rows_written, recording_format=recording_format
        )
        ...
        while True:
            try:
                ...
                if shm_reader and shm_queue is not None:
                    ...
                    if monitored_names and (now - last_vars_update_at >= interval_between_updates):
                        ...
                        try:
                            force_full_now = force_full_vars_update or (
                                full_refresh_sec > 0.0 and (now - last_full_vars_update_at) >= full_refresh_sec
                            )
            ...
            try:
                ...
                # Historial en vivo eliminado: se mantiene solo vía SHM y grabaciones TSV.
                    ...
                    record_buffer.clear()
                    recording_var_names = [
                        n for n in list(monitored_names) if n not in VARMON_TELEMETRY_NAME_SET
                    ] if monitored_names else None
                    ...
                    bin_path = _resolve_recording_sidecar_bin(_config) if want_sidecar else None
                    if want_sidecar and not bin_path:
                        # Fallback automático a writer Python (TSV) si el sidecar no está disponible.
                        # Evita bloquear la grabación cuando el formato elegido no es Parquet.
                        want_sidecar = False
                        print(
                            "[VarMonitor Web] Grabación sidecar: ejecutable no encontrado; "
                            "fallback a grabación Python (TSV).",
                            flush=True,
                        )
                    if want_sidecar:
                        _ensure_recordings_dir()
                        ...
                        recording_sidecar_alarm_exit_path = None
                        recording_sidecar_alarm_delegate = False
                        nv = [n for n in list(monitored_names) if n not in VARMON_TELEMETRY_NAME_SET] if monitored_names else []
                        ...
                        try:
                                ...
                                if sidecar_poll_hz > 0:
                                    args_base.extend(["--poll-hz", str(sidecar_poll_hz)])
...
_bootstrap_web_static_from_seed()
_merge_static_seed_client_gaps()
...
_REPO_ROOT_FOR_DOCS = Path(__file__).resolve().parent.parent


def _pick_mkdocs_site_dir(subdir: str) -> str:
    """site (ES) o site_en (EN): prioriza ./VarMonitor junto al monitor, luego la raíz del monitor."""
    for base in (_REPO_ROOT_FOR_DOCS / "VarMonitor", _REPO_ROOT_FOR_DOCS):
        p = base / subdir
        if p.is_dir():
            return str(p)
    return str(_REPO_ROOT_FOR_DOCS / "VarMonitor" / subdir)


# Salida de mkdocs build (ES/EN); generar desde el repo de documentación (carpeta VarMonitor/)
_DOCS_ES_DIR = _pick_mkdocs_site_dir("site")
_DOCS_EN_DIR = _pick_mkdocs_site_dir("site_en")
...
_DOCS_NOT_BUILT_HTML = """<!DOCTYPE html><html><head><meta charset="utf-8"><title>Documentation</title></head><body style="font-family:sans-serif;padding:2rem;max-width:640px">
    ...
    <p>Build the MkDocs static sites (Spanish <code>site/</code>, English <code>site_en/</code>) from the
    <strong>VarMonitor</strong> documentation repository, e.g. clone it next to this repo as <code>VarMonitor/</code>,
    then from that folder:</p>
    <pre>pip install mkdocs mkdocs-material pymdown-extensions
mkdocs build
mkdocs build -f mkdocs.en.yml</pre>
    <p>Restart the monitor. Spanish will be at <a href="/docs/es/">/docs/es/</a>, English at <a href="/docs/en/">/docs/en/</a>.</p>
    <p>Public copy (GitHub Pages): <a href="https://lorenzoadr.github.io/VarMonitor/docs/es/">/docs/es/</a> and
    <a href="https://lorenzoadr.github.io/VarMonitor/docs/en/">/docs/en/</a>.</p>
    </body></html>"""
