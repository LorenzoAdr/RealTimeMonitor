...
export const ARINC_CSV_TEMPLATE =
    ...
    'General,310,IAS_EXAMPLE,bnr,19,,1,false,kt,0,450,"3"\n';

/** Definiciones demo integradas (mismo contrato que el registro de usuario). */
export const ARINC_BUILTIN_LABEL_DEFS = {
    "203": { name: "PITCH_ANGLE_DEMO", encoding: "bnr", signed: true, bits: 19, scale: 1, units: "deg", min: -90, max: 90, ssmAllowed: [3] },
    "204": { name: "ROLL_DEMO", encoding: "bnr", signed: true, bits: 19, scale: 0.01, units: "deg", min: -90, max: 90, ssmAllowed: [3] },
    "206": { name: "HEADING_DEMO", encoding: "bnr", signed: false, bits: 19, scale: 0.01, units: "deg", min: 0, max: 360, ssmAllowed: [3] },
    "310": { name: "IAS_DEMO", encoding: "bnr", signed: false, bits: 19, scale: 1, units: "kt", min: 0, max: 450, ssmAllowed: [3] },
    "271": { name: "ALTITUDE_BCD_DEMO", encoding: "bcd", signed: false, bits: 19, scale: 1, units: "ft", min: -1000, max: 60000, ssmAllowed: [0, 3] },
    "320": { name: "CAS_DEMO", encoding: "bnr", signed: false, bits: 19, scale: 0.0625, units: "kt", min: 0, max: 500, ssmAllowed: [3] },
    "350": {
        name: "STATUS_DIS_DEMO",
        encoding: "discrete",
        bits: 19,
        /* Solo SSM=0 válido para DIS; sin esto el default [0..3] aceptaba SSM=3 y no se veía fallo en demo. */
        ssmAllowed: [0],
        discreteBits: [
            { index: 0, name: "ON_GROUND" },
            { index: 1, name: "IN_FLIGHT" },
            { index: 2, name: "OVERSPEED" },
            { index: 3, name: "MAINT" },
            { index: 4, name: "WOW_MAIN" },
        ],
    },
...
/** Extrae el label octal (3 dígitos) desde una clave de registro `Grupo::oct` o `oct`. */
function registryKeyToStubOctKey(regKey) {
    const s = String(regKey || "");
    const m = /^(.+)::([0-7]{3})$/.exec(s);
    if (m) return m[2];
    if (/^[0-7]{3}$/.test(s)) return s;
    return s;
}

function registryStorageKeyForGroupOct(group, oct) {
...
export function mergeRegistries(existing, incoming, mode) {
    ...
    for (const [oct, inc] of Object.entries(incoming || {})) {
        ...
        if (
            ...
            isDisImportStub(inc, registryKeyToStubOctKey(oct)) &&
...
export function buildRegistryFromMappedRows(headers, rows, mapping, importMode, labelRadix) {
    ...
    if (importMode === "dis_bits") {
        ...
        for (const row of rows) {
            ...
            if (!oct) continue;
            const groupRaw = inv.group_name ? String(rowGet(row, inv.group_name) || "").trim() : "";
            const group = groupRaw || "General";
            const key = registryStorageKeyForGroupOct(group, oct);
            ...
            if (!bitsByKey[key]) bitsByKey[key] = [];
            bitsByKey[key].push({ index: Math.floor(bi), name: bn });
        }
        const out = {};
        for (const [key, bits] of Object.entries(bitsByKey)) {
            const m = /^(.+)::([0-7]{3})$/.exec(key);
            const group = m ? m[1] : "General";
            const oct = m ? m[2] : key;
            out[key] = normalizeLabelEntry({
                group,
                labelOct: oct,
                name: `DIS_${oct}`,
                encoding: "discrete",
...
export function guessMappingFromHeaders(headers, savedMap) {
    ...
    const patterns = [
        ...
        { id: "dis_bit_index", keys: ["bit_index", "bit_idx", "bit_number", "bdis", "bit_num", "num_bit", "bit_no", "bit"] },
        { id: "dis_bit_name", keys: ["bit_name", "bitname", "discrete_name", "nombre_bit"] },
    ];
    const map = {};
