...
def mavlink_health_scan_state_dir() -> str:
    """Directorio para `mavlink_health_scan_*.json` (misma regla que `MavlinkLinkHealth::server_state_dir` en CoreNexus)."""
    env = (os.environ.get("VARMON_SERVER_STATE_DIR") or "").strip()
    if env:
        return os.path.abspath(os.path.expanduser(env))
    cfg = (os.environ.get("VARMON_CONFIG") or "").strip()
    if cfg:
        base = os.path.dirname(os.path.abspath(os.path.expanduser(cfg)))
        return os.path.abspath(os.path.join(base, "server_state"))
    return os.path.abspath(STATE_ROOT_DIR)
...
print(
    ...
    f"mavlink_health_scan={mavlink_health_scan_state_dir()}",
    flush=True,
)
print(
