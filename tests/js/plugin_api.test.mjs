...
import {
    ...
    getHooks,
    _reset,
} from "../../web_monitor/client_oss/js/modules/plugin_api.mjs";
...
describe("registerPlugin + hasPlugin", () => {
    it("registers allowlisted plugin", () => {
        registerPlugin("replay_alias", { version: "1.0" });
        expect(hasPlugin("replay_alias")).toBe(true);
    });

    it("ignores plugins outside legacy allowlist", () => {
        registerPlugin("arinc", { version: "1.0" });
        expect(hasPlugin("arinc")).toBe(false);
    });
...
describe("getRegisteredPluginIds", () => {
    it("returns sorted registered plugin IDs (allowlist only)", () => {
        registerPlugin("parquet");
        registerPlugin("arinc");
        registerPlugin("segments");
        expect(getRegisteredPluginIds()).toEqual(["segments"]);
    });
...
describe("getPluginMeta", () => {
    it("returns meta", () => {
        registerPlugin("anomaly", { author: "test", version: "2.0" });
        expect(getPluginMeta("anomaly")).toEqual({ author: "test", version: "2.0" });
    });
...
describe("registerAppMode", () => {
    it("registers app mode only for allowlisted ids", () => {
        registerAppMode("segments", { labelFallback: "Segments", bodyClass: "mode-segments" });
        expect(hasRegisteredAppMode("segments")).toBe(true);
        expect(getRegisteredAppModeIds()).toContain("segments");
        registerAppMode("file_edit", { labelFallback: "Files", bodyClass: "mode-file-edit" });
        expect(hasRegisteredAppMode("file_edit")).toBe(false);
    });
});
