...
![VarMonitor web UI — light theme (live mode: browser, monitor, plots)](pictures/general_claro.png)

> **Documentation** (MkDocs + Astro landing) lives in the separate **[VarMonitor](https://github.com/LorenzoAdr/VarMonitor)** repo and is published at **`https://lorenzoadr.github.io/VarMonitor/`**. Clone it next to this tree as `VarMonitor/` if you want to build `site/` for the monitor’s **Docs** button. This README stays in English for the default GitHub project page.
...
See **[Docker (ES)](https://github.com/LorenzoAdr/VarMonitor/blob/main/docs/docker.md)** or **[Docker (EN)](https://github.com/LorenzoAdr/VarMonitor/blob/main/docs_en/docker.md)**. To embed the monitor in another image without depending on submodule paths, install the three runtime packages listed in **`web_monitor/requirements-docker.txt`** via `RUN pip install ...` in your Dockerfile (see docs); that file is the version reference.

## Standalone binary (no Python on target)

Build a single executable with PyInstaller (details: **[build-binary (EN)](https://github.com/LorenzoAdr/VarMonitor/blob/main/docs_en/build-binary.md)**):
...
To run the PyInstaller build: **`export VARMON_PACKAGED_WEB_BIN=.../varmonitor-web`** then **`./scripts/launch_web.sh`**; open the UI with **`./scripts/launch_ui.sh`** (needs `python3` for these launcher scripts only; see [build-binary (EN)](https://github.com/LorenzoAdr/VarMonitor/blob/main/docs_en/build-binary.md)).
...
- **C++ publisher tuning**: `shm_publish_dirty_mode`, `shm_publish_full_refresh_cycles`, `shm_publish_skip_unchanged`, `shm_publish_slice_count`, `shm_async_publish` — see **[Performance (EN)](https://github.com/LorenzoAdr/VarMonitor/blob/main/docs_en/performance.md)**.
...
Screenshots in [`pictures/`](pictures/) (copies of the figures used in the MkDocs sources under **VarMonitor**).

| Live — dark theme |
|-------------------|
| ![VarMonitor UI, dark theme](pictures/general_oscuro2.png) |

| Analysis mode (offline TSV / Parquet) | Replay mode (TSV + live SHM) |
|-----------------------------|------------------------------|
| ![Analysis mode](pictures/analisis.png) | ![Replay mode](pictures/replay.png) |

| Advanced plot tools | Perf panel (`/api/perf`) |
|---------------------|-------------------------|
| ![Advanced options](pictures/avanzado.png) | ![Performance overlay](pictures/perf.png) |

| Help (in-app guide) | Built-in log viewer |
|---------------------|---------------------|
| ![Help modal](pictures/manual.png) | ![Log panel](pictures/log.png) |
...
│   └── static/
├── pictures/            # README / marketing screenshots (MkDocs figures live in repo VarMonitor)
├── LorenzoAdr.github.io/   # User/org GitHub Pages site (separate from project docs)
└── scripts/
```

MkDocs sources, Astro landing, and the **Deploy Astro** workflow: separate Git repo **[VarMonitor](https://github.com/LorenzoAdr/VarMonitor)** (clone as `VarMonitor/` next to this tree; not tracked here).
