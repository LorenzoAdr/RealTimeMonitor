...
En el destino, coloca `varmonitor-web`, `varmon_sidecar` y `varmon.conf` en el mismo directorio de instalación. Por defecto, grabaciones y estado van a `INSTALL_DIR/data/recordings/` y `INSTALL_DIR/data/server_state/` (override con `VARMON_DATA_DIR` o claves en `varmon.conf`).
