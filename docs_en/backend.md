...
- **SHM layout**: Same as [web_monitor/shm_reader.py](../web_monitor/shm_reader.py) (32-byte header, 137 bytes per variable entry).
- **CLI**: **Recording:** `--shm-name`, `--sem-name`, `--output` (`.part` file), `--names-file`, `--max-vars`, optional `--status-file`, optional `--alarms-file` (rules TSV) and `--alarm-exit-file`. **Live alarm monitor:** `--alarm-monitor` with `--names-file`, `--alarms-file`, `--alarm-events-file` (NDJSON append), `--alarm-output-dir` (where `alarm_*.tsv` is written); does not use the SHM semaphore (polls `seq`).
