...
The `web_monitor_version/` layout is: `bin/` (`varmonitor-web`, `varmon_sidecar`, `libvarmonitor.so*`), `data/` (`varmon.conf`), `include/` (public C++ headers for linking against the shared library). With `source scripts/simple_config.sh` in `package` mode, defaults point at `INSTALL_DIR/bin/`, `INSTALL_DIR/data/varmon.conf`, and `INSTALL_DIR/data/` for recordings and state (override with `VARMON_DATA_DIR` or keys in `varmon.conf`).
