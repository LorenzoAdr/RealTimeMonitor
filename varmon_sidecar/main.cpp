...
/** Tolerancia absoluta (s) para considerar dos timestamps de cabecera/muestra como el mismo instante lógico. */
static constexpr double kTsDupAbsTolSec = 1e-6;

/** Mismo instante lógico en cabecera/muestra (doble): no emitir otra fila TSV si el timestamp es idéntico salvo ruido FP.
 *  Nota: no escalar por |epoch|; con wall-clock Unix ~1e9, 1e-9*|a| ~ 1 s y colapsaba ~1 fila/segundo. */
static bool ts_duplicate_for_tsv(double a, double b) {
    if (!std::isfinite(a) || !std::isfinite(b)) return false;
    return std::abs(a - b) <= kTsDupAbsTolSec;
}
...
/** VARMON_SIDECAR_RECORDING_TRACE: 0 off, 1 líneas resumen, 2 verbose (hex/%.17g, dedup, cada wake). */
static int g_recording_trace_level_cached = -1;

static int recording_trace_level_parse_once() {
    const char* v0 = std::getenv("VARMON_SIDECAR_RECORDING_TRACE");
    if (v0 == nullptr) return 0;
    while (*v0 == ' ' || *v0 == '\t') ++v0;
    if (*v0 == '\0') return 0;
    char* end = nullptr;
    const long n = std::strtol(v0, &end, 10);
    if (end != v0) {
        while (*end == ' ' || *end == '\t') ++end;
        if (*end == '\0') {
            if (n >= 2) return 2;
    ...
    if (strcasecmp(v0, "verbose") == 0 || strcasecmp(v0, "v") == 0 || strcasecmp(v0, "debug") == 0) return 2;
    if (strcasecmp(v0, "true") == 0 || strcasecmp(v0, "yes") == 0 || strcasecmp(v0, "on") == 0) return 1;
    if (strcasecmp(v0, "false") == 0 || strcasecmp(v0, "no") == 0 || strcasecmp(v0, "off") == 0) return 0;
    return 1;
}

static int recording_trace_level() {
    if (g_recording_trace_level_cached < 0) {
        g_recording_trace_level_cached = recording_trace_level_parse_once();
    }
    return g_recording_trace_level_cached;
}

static bool recording_trace_enabled() { return recording_trace_level() >= 1; }

static bool recording_trace_verbose() { return recording_trace_level() >= 2; }
...
struct AlarmRule {
    std::string name;
    /** Si true: |live(name) − ref(ref_name)| vs tol (histéresis hys). */
    bool ref_mode = false;
    std::string ref_name;
    double tol = 0;
    bool has_tol = false;
...
bool read_alarms_file(const std::string& path, std::vector<AlarmRule>& out) {
    ...
    while (std::getline(f, line)) {
        ...
        /* v2: name\tabs|ref\t... (6 campos) */
        if (p.size() >= 6 && (p[1] == "abs" || p[1] == "ref")) {
            AlarmRule r;
            r.name = p[0];
            if (p[1] == "abs") {
                r.ref_mode = false;
                if (!parse_opt_double(p[2], r.lo, r.has_lo)) continue;
                if (!parse_opt_double(p[3], r.hi, r.has_hi)) continue;
            } else {
                r.ref_mode = true;
                r.ref_name = p[2];
                if (r.ref_name.empty()) continue;
                if (!parse_opt_double(p[3], r.tol, r.has_tol)) continue;
                if (!r.has_tol) continue;
            }
            bool has_hys = false;
            if (!parse_opt_double(p[4], r.hys, has_hys)) continue;
            if (!has_hys) r.hys = 0.0;
            r.hys = std::max(0.0, r.hys);
            if (!parse_opt_int(p[5], r.delay_ms)) continue;
            if (r.delay_ms < 0) r.delay_ms = 0;
            out.push_back(std::move(r));
            continue;
        }
        /* v1 legado: name\tlo\thi\thys\tdelay_ms */
...
bool evaluate_alarms_step(const std::vector<AlarmRule>& rules, const std::unordered_map<std::string, Entry>& snap,
    ...
    for (const auto& rule : rules) {
        ...
        if (rule.ref_mode) {
            auto it_ref = snap.find(rule.ref_name);
            if (it_ref == snap.end()) continue;
            const double v_live = entry_numeric_for_alarm(it->second);
            const double v_ref = entry_numeric_for_alarm(it_ref->second);
            metric = std::abs(v_live - v_ref);
            val = metric;
            const double tol = rule.tol;
            const bool over = metric > tol;
            if (was) {
                if (metric <= tol - hys) {
                    alarming = false;
                } else {
                    alarming = metric > (tol - hys);
                }
            } else {
                alarming = over;
            }
        } else {
            val = entry_numeric_for_alarm(it->second);
            metric = val;
        ...
        if (!alarming) {
            ...
            if (confirmed && !was) {
                ...
                if (rule.ref_mode) {
                    rs << rule.name << " |live-ref|=" << std::setprecision(4) << metric << " > tol:" << std::setprecision(4)
                       << rule.tol;
                } else {
                    const double hi = rule.has_hi ? rule.hi : 0.0;
                    const double lo = rule.has_lo ? rule.lo : 0.0;
                ...
                out_name = rule.name;
                out_reason = rs.str();
                out_value = rule.ref_mode ? metric : val;
                return true;
            }
        }
    }
...
/** Tras un wait exitoso: vacía posts POSIX acumulados. El mmap solo refleja el último snapshot; si no
 * drenamos, cada post consume un ciclo pero seq/ts visibles son los del último publish → skip_same_shm_seq
 * en bucle y el TSV queda con una sola fila. Misma idea que shm_reader.py (drenar antes de leer). */
static void drain_sem_accumulated_posts(sem_t* sem) {
    if (!sem) return;
...
int main(int argc, char** argv) {
    ...
    /** Último instante ya escrito en TSV (timestamp cabecera SHM o muestra anillo); evita filas repetidas para el mismo tiempo. */
    double last_emitted_logical_ts = std::numeric_limits<double>::quiet_NaN();
    uint64_t rows = 0;
    ...
    {
        const int rtl = recording_trace_level();
        const char* raw = std::getenv("VARMON_SIDECAR_RECORDING_TRACE");
        std::fprintf(stderr,
                     "[sidecar_recording_trace] startup: effective_level=%d getenv=%s (stderr del proceso sidecar; "
                     "nivel 2 → líneas sidecar_recording_trace2)\n",
                     rtl, raw != nullptr ? raw : "(null)");
        std::fflush(stderr);
    }
    ...
    while (!g_stop.load()) {
...
#if VARMON_SIDECAR_WAKE_TRACE
        {
            ...
            double ts_hdr_wake = std::numeric_limits<double>::quiet_NaN();
            if (map_size >= 32) {
                std::memcpy(&ts_hdr_wake, base + 24, 8);
            }
            const double w = std::chrono::duration<double>(std::chrono::system_clock::now().time_since_epoch())
                                 .count();
            std::fprintf(stderr,
                         "[sidecar_trace] wake  wall_s=%.6f delta_prev_wake_ms=%.3f wake#=%llu seq=%llu ts_hdr@+24=%.9f\n",
                         w, d_wake_ms, static_cast<unsigned long long>(wake_cycle),
                         static_cast<unsigned long long>(cur_seq), ts_hdr_wake);
            std::fflush(stderr);
            trace_prev_wake = trace_this_wake;
            trace_have_prev_wake = true;
        }
#endif
        if (recording_trace_verbose()) {
            double ts_hdr_raw = 0.0;
            if (map_size >= 32) {
                std::memcpy(&ts_hdr_raw, base + 24, 8);
            }
            std::fprintf(stderr,
                         "[sidecar_recording_trace2] wake#%llu seq=%llu have_last=%d last_seq=%llu will_skip_dup_seq=%d "
                         "ts_header@+24=%.17g\n",
                         static_cast<unsigned long long>(wake_cycle), static_cast<unsigned long long>(cur_seq),
                         have_seq ? 1 : 0, static_cast<unsigned long long>(last_seq),
                         (have_seq && cur_seq == last_seq) ? 1 : 0, ts_hdr_raw);
            std::fflush(stderr);
        }
        if (ring_trace_enabled()) {
        ...
        if (have_seq && cur_seq == last_seq) {
            if (recording_trace_enabled()) {
                std::fprintf(stderr,
                             "[sidecar_recording_trace] skip_same_shm_seq cur_seq=%llu last_seq=%llu (sin avance → sin "
                             "fila nueva)\n",
                             static_cast<unsigned long long>(cur_seq), static_cast<unsigned long long>(last_seq));
                std::fflush(stderr);
            }
#if VARMON_SIDECAR_WAKE_TRACE
            {
                ...
                std::fprintf(stderr,
                             "[sidecar_trace] no_row reason=same_shm_seq wall_s=%.6f delta_prev_done_ms=%.3f "
                             "since_wake_ms=%.3f seq=%llu rows=%llu ring_replay=0\n",
                             w, d_done_ms, since_wake_ms, static_cast<unsigned long long>(cur_seq),
                             static_cast<unsigned long long>(rows));
                std::fflush(stderr);
        ...
        if (recording_trace_enabled()) {
            std::fprintf(stderr,
                         "[sidecar_recording_trace] after_extract replay_n=%d cur_seq=%llu (replay_n>0 → ruta anillo "
                         "v2)\n",
                         replay_n, static_cast<unsigned long long>(cur_seq));
            std::fflush(stderr);
        }
        ...
        if (replay_n > 0) {
            ...
            /* Un sem_timedwait = un ciclo: aunque el anillo v2 drene varias muestras pendientes en una pasada,
               solo se persiste una fila TSV por despertar (la más reciente, o la que dispara alarma si aplica). */
            int alarm_fire_ri = -1;
            std::string alarm_an;
            std::string alarm_ar;
            double alarm_av = 0.0;
            auto t_al0 = sc_clock::now();
            if (!alarm_rules.empty()) {
                for (int ri = 0; ri < replay_n; ++ri) {
                    ...
                    if (evaluate_alarms_step(alarm_rules, snap, now_ms, prev_alarm, pending_alarm, an, ar, av)) {
                        alarm_fire_ri = ri;
                        alarm_an = std::move(an);
                        alarm_ar = std::move(ar);
                        alarm_av = av;
                        break;
                    }
                }
            }
            acc_al += SidecarPerf::micros(t_al0, sc_clock::now());
            const int out_ri = (alarm_fire_ri >= 0) ? alarm_fire_ri : (replay_n - 1);
            const std::string& line = replay_lines[static_cast<size_t>(out_ri)].second;
            const double ring_logical_ts = replay_lines[static_cast<size_t>(out_ri)].first;
            const bool dup_ring_ts = (alarm_fire_ri < 0) && std::isfinite(last_emitted_logical_ts) &&
                                       ts_duplicate_for_tsv(ring_logical_ts, last_emitted_logical_ts);
            if (recording_trace_verbose()) {
                const double ad =
                    (std::isfinite(ring_logical_ts) && std::isfinite(last_emitted_logical_ts))
                        ? std::abs(ring_logical_ts - last_emitted_logical_ts)
                        : -1.0;
                const double tol = std::isfinite(ring_logical_ts) ? kTsDupAbsTolSec : -1.0;
                std::fprintf(stderr,
                             "[sidecar_recording_trace2] RING_DEDUP wake#%llu seq=%llu replay_n=%d alarm_ri=%d "
                             "ts_sample=%.17g last_emit=%.17g abs_diff=%.17g tol=%.17g -> dup_ring=%d\n",
                             static_cast<unsigned long long>(wake_cycle), static_cast<unsigned long long>(cur_seq),
                             replay_n, alarm_fire_ri, ring_logical_ts, last_emitted_logical_ts, ad, tol,
                             dup_ring_ts ? 1 : 0);
                std::fflush(stderr);
            }
            if (dup_ring_ts && recording_trace_enabled()) {
                std::fprintf(stderr,
                             "[sidecar_recording_trace] skip_dup_ring_sample_ts ts=%.12g seq=%llu replay_n=%d\n",
                             ring_logical_ts, static_cast<unsigned long long>(cur_seq), replay_n);
                std::fflush(stderr);
            }
#if VARMON_SIDECAR_WAKE_TRACE
            if (dup_ring_ts) {
                ...
                const double since_wake_ms =
                    std::chrono::duration<double, std::milli>(now_d - trace_this_wake).count();
                std::fprintf(stderr,
                             "[sidecar_trace] no_row reason=dup_ring_ts wall_s=%.6f delta_prev_done_ms=%.3f "
                             "since_wake_ms=%.3f seq=%llu replay_n=%d ts_sample=%.9f rows=%llu ring_replay=1\n",
                             w, d_done_ms, since_wake_ms, static_cast<unsigned long long>(cur_seq), replay_n,
                             ring_logical_ts, static_cast<unsigned long long>(rows));
                std::fflush(stderr);
                trace_prev_done = now_d;
                trace_have_prev_done = true;
            }
            ...
            if (!dup_ring_ts && std::fwrite(line.data(), 1, line.size(), out) != line.size()) {
                ...
                ++rows;
                last_emitted_logical_ts = ring_logical_ts;
                if (recording_trace_enabled()) {
                    std::fprintf(stderr,
                                 "[sidecar_recording_trace] RING_TSV_ROW rows_total=%llu cur_seq=%llu replay_n=%d "
                                 "out_ri=%d alarm_ri=%d ts_sample=%.9f\n",
                                 static_cast<unsigned long long>(rows), static_cast<unsigned long long>(cur_seq),
                                 replay_n, out_ri, alarm_fire_ri, ring_logical_ts);
                    std::fflush(stderr);
                }
#if VARMON_SIDECAR_WAKE_TRACE
                {
                    ...
                    std::fprintf(stderr,
                                 ...
                                 "rows=%llu ring_replay=1 seq=%llu replay_n=%d ts_sample=%.9f out_ri=%d alarm_ri=%d\n",
                                 w, d_done_ms, since_wake_ms, static_cast<unsigned long long>(rows),
                                 static_cast<unsigned long long>(cur_seq), replay_n, ring_logical_ts, out_ri,
                                 alarm_fire_ri);
                    std::fflush(stderr);
                    trace_prev_done = now_d;
                    trace_have_prev_done = true;
...
#endif
                if (alarm_fire_ri >= 0) {
                    write_alarm_exit_json(alarm_exit_path, alarm_an, alarm_ar, alarm_av);
                    last_status_rows = rows;
                    write_status();
                    recording_break = true;
                } else if (!status_path.empty() && (rows - last_status_rows >= 64 || rows == 1)) {
                    last_status_rows = rows;
                    write_status();
                }
            }
        ...
        const bool dup_header_ts =
            std::isfinite(last_emitted_logical_ts) && ts_duplicate_for_tsv(ts, last_emitted_logical_ts);
        if (recording_trace_verbose()) {
            const double adiff =
                (std::isfinite(ts) && std::isfinite(last_emitted_logical_ts))
                    ? std::abs(ts - last_emitted_logical_ts)
                    : -1.0;
            const double tol = std::isfinite(ts) ? kTsDupAbsTolSec : -1.0;
            std::fprintf(stderr,
                         "[sidecar_recording_trace2] SNAP_DEDUP wake#%llu seq=%llu ts_hdr=%.17g last_emit=%.17g "
                         "abs_diff=%.17g tol=%.17g -> dup_header=%d replay_n_was_0 path=snapshot\n",
                         static_cast<unsigned long long>(wake_cycle), static_cast<unsigned long long>(cur_seq), ts,
                         last_emitted_logical_ts, adiff, tol, dup_header_ts ? 1 : 0);
            std::fflush(stderr);
        }
        if (dup_header_ts && recording_trace_enabled()) {
            std::fprintf(stderr, "[sidecar_recording_trace] skip_dup_header_ts ts=%.12g seq=%llu\n", ts,
                         static_cast<unsigned long long>(cur_seq));
            std::fflush(stderr);
        }
#if VARMON_SIDECAR_WAKE_TRACE
        if (dup_header_ts) {
            const auto now_d = trace_steady::now();
            const double w = std::chrono::duration<double>(std::chrono::system_clock::now().time_since_epoch()).count();
            double d_done_ms = -1.0;
            if (trace_have_prev_done) {
                d_done_ms = std::chrono::duration<double, std::milli>(now_d - trace_prev_done).count();
            }
            ...
            const double adiff =
                (std::isfinite(ts) && std::isfinite(last_emitted_logical_ts))
                    ...
                    : -1.0;
            std::fprintf(stderr,
                         "[sidecar_trace] no_row reason=dup_header_ts wall_s=%.6f delta_prev_done_ms=%.3f "
                         "since_wake_ms=%.3f seq=%llu ts_hdr=%.9f abs_diff_vs_last_emit=%.9g rows=%llu ring_replay=0\n",
                         w, d_done_ms, since_wake_ms, static_cast<unsigned long long>(cur_seq), ts, adiff,
                         static_cast<unsigned long long>(rows));
            std::fflush(stderr);
            trace_prev_done = now_d;
            trace_have_prev_done = true;
        ...
        sc_clock::time_point t_sf0_snap{};
        sc_clock::time_point t_sf_mid_snap{};
        sc_clock::time_point t_sf1_snap{};
        sc_clock::time_point t_w0_snap{};
        sc_clock::time_point t_w1_snap{};

        if (!dup_header_ts) {
            const double t_rel = row_time_s(ts);
            if (recording_trace_verbose()) {
                std::fprintf(stderr,
                             "[sidecar_recording_trace2] row_time_s: ts_in=%.17g t_rel=%.17g last_t_rel_written=%.17g "
                             "t_epoch=%.17g\n",
                             ts, t_rel, last_t_rel_written, t_epoch);
                std::fflush(stderr);
            }
            t_sf0_snap = sc_clock::now();
            std::string line;
            t_sf_mid_snap = t_sf0_snap;
            t_sf1_snap = t_sf0_snap;
            if (used_column_cache) {
                append_recording_tsv_line_from_cols(line, t_rel, col_names, column_entries, col_present);
                t_sf_mid_snap = sc_clock::now();
                t_sf1_snap = t_sf_mid_snap;
            } else {
                line.reserve(col_names.size() * 24u + 48u);
                char tbuf[32];
                ...
                line.push_back('\n');
                t_sf1_snap = sc_clock::now();
            }
            if (sc_perf) {
                sc_perf->record(14, SidecarPerf::micros(t_sf0_snap, t_sf_mid_snap));
                sc_perf->record(15, SidecarPerf::micros(t_sf_mid_snap, t_sf1_snap));
                sc_perf->record(7, SidecarPerf::micros(t_sf0_snap, t_sf1_snap));
            }
            t_w0_snap = sc_clock::now();
            if (std::fwrite(line.data(), 1, line.size(), out) != line.size()) {
                std::fprintf(stderr, "escritura fila falló\n");
                if (sc_perf) {
                    sc_perf->record(8, SidecarPerf::micros(t_w0_snap, sc_clock::now()));
            ...
            t_w1_snap = sc_clock::now();
            if (sc_perf) sc_perf->record(8, SidecarPerf::micros(t_w0_snap, t_w1_snap));
            if (sc_perf) sc_perf->record(17, SidecarPerf::micros(t_cycle_anchor, t_w1_snap));
            ++rows;
            last_emitted_logical_ts = ts;
            if (recording_trace_enabled()) {
                std::fprintf(stderr,
                             "[sidecar_recording_trace] SNAP_TSV_ROW rows_total=%llu cur_seq=%llu header_ts=%.9f "
                             "t_rel=%.9f used_col_cache=%d\n",
                             static_cast<unsigned long long>(rows), static_cast<unsigned long long>(cur_seq), ts, t_rel,
                             used_column_cache ? 1 : 0);
                std::fflush(stderr);
            }
#if VARMON_SIDECAR_WAKE_TRACE
            {
                ...
                std::fprintf(stderr,
                             ...
                             "ring_replay=0 seq=%llu ts_hdr=%.9f t_rel=%.9f path=snapshot\n",
                             w, d_done_ms, since_wake_ms, static_cast<unsigned long long>(rows),
                             static_cast<unsigned long long>(cur_seq), ts, t_rel);
                std::fflush(stderr);
                trace_prev_done = now_d;
                trace_have_prev_done = true;
            }
        ...
        if (sc_perf && !dup_header_ts) {
            ...
            const uint64_t us14s = SidecarPerf::micros(t_sf0_snap, t_sf_mid_snap);
            const uint64_t us15s = SidecarPerf::micros(t_sf_mid_snap, t_sf1_snap);
            const uint64_t us8s = SidecarPerf::micros(t_w0_snap, t_w1_snap);
            const uint64_t us17snap = SidecarPerf::micros(t_cycle_anchor, t_w1_snap);
            const double sum_tf =
