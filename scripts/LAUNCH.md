...
## `scripts/config.sh` (modo y rutas)
...
- En shell interactivo: `source scripts/config.sh` y luego los comandos que quieras.
...
| `./scripts/launch_corenexus.sh` | **corenexus** (hub VarMonitor / SHM+UDS). Opción `--build` para compilar antes (`scripts/varmon/build_corenexus.sh`). En otra terminal: `launch_web.sh`. Modo `package`: **`VARMON_CORENEXUS_BIN`**. Pruebas MAVLink sin hardware: `launch_mavlink_udp.sh`, `launch_mavlink_serie.sh`, `launch_corenexus_both.sh`, `launch_mavlink_emitter_serial.sh` (emisor al PTY _b_; ver [`CoreNexus/testing/README.md`](../CoreNexus/testing/README.md)). |
...
| `./scripts/stop_varmonitor.sh` | Detiene procesos VarMonitor del **usuario actual** (demo_server, **corenexus**, backend web, varmonitor-web, sidecar, emisores de prueba MAVLink en `scripts/` o `web_monitor_version/testing/`; patrones acotados). |
...
| `./scripts/run_robustness_tests.sh` | **Smoke** de la batería de robustez MAVLink/SHM: `py_compile` del emisor, pytest del fragmento `scripts/robustness/varmon_robustness.frag`. No levanta corenexus. Ver [`scripts/robustness/README.md`](robustness/README.md). |
| `./scripts/launch_mavlink_robustness.sh` | Emisor MAVLink con perfiles `corrupt`/`truncate`/`burst`/… ([`mavlink_robustness_emitter.py`](mavlink_robustness_emitter.py)). |
| `./scripts/launch_mavlink_generator_gui.sh` | GUI Tkinter unificada para generar MAVLink (UDP/serie, socat PTY, `drone`/`legacy`) y cambiar perfiles de error en caliente sin reiniciar el proceso. |

## `scripts/varmon/` (auxiliar)

Incluye: `config.sh` (modo `code`/`package` y exports), `setup.sh`, `docker-run.sh`, `build_varmonitor_web.sh`, `build_web_static_js.sh` (genera `static/dist/app.bundle.min.js` desde `static/js/entry.mjs` con esbuild/npx), `build_corenexus.sh`, `generate_webmonitor_version.sh` (empaqueta `web_monitor_version/` con sidecar + PyInstaller + opcionalmente JS y binarios demo/corenexus si se piden), `gui_plugins_deploy.py` (GUI tkinter: plugins + opciones **Compilar demo_server / corenexus** en entrega final; requiere `python3-tk` en muchos Linux), `patch_pywebview_qt.py`, módulos `varmon_*.py`, `launch_*.py`, `build_docs_pdf.py`.
