...
Incluye: `simple_config.sh` (modo `code`/`package` y exports), `setup.sh`, `docker-run.sh`, `build_varmonitor_web.sh`, `build_web_static_js.sh` (minifica `static/app.js` con esbuild/npx), `generate_webmonitor_version.sh` (empaqueta `web_monitor_version/` con sidecar + PyInstaller + opcionalmente JS), `patch_pywebview_qt.py`, módulos `varmon_*.py`, `launch_*.py`, `build_docs_pdf.py`.
...
./scripts/varmon/generate_webmonitor_version.sh   # entrega: web_monitor_version/
```
