...
if [[ "$RUN_PY" -eq 1 ]]; then
  ...
  if [[ ! -f "$VENV" ]]; then
    ...
    source "$VENV"
    HAS_TOOL_PLUGINS_PY=0
    if [[ -d "$ROOT/tool_plugins/python" ]]; then
      HAS_TOOL_PLUGINS_PY=1
      # Paquete Pro: necesario para rutas y tests en tool_plugins/python/tests.
    ...
    if [[ "$COVERAGE" -eq 1 ]]; then
      ...
      COV_ARGS=(--cov=perf_agg --cov=uds_client --cov=shm_reader --cov=app --cov-report=term-missing)
      if [[ "$HAS_TOOL_PLUGINS_PY" -eq 1 ]]; then
        COV_ARGS=(--cov=perf_agg --cov=uds_client --cov=shm_reader --cov=varmonitor_plugins --cov=app --cov-report=term-missing)
      fi
    fi
    PY_FAIL=0
    # Tests Pro (solo si existe el árbol tool_plugins).
    if [[ "$HAS_TOOL_PLUGINS_PY" -eq 1 && -d "$ROOT/tool_plugins/python/tests" ]]; then
      if ! python -m pytest "$ROOT/tool_plugins/python/tests/" -v "${COV_ARGS[@]}"; then
        PY_FAIL=1
      fi
    elif [[ "$HAS_TOOL_PLUGINS_PY" -eq 0 ]]; then
      echo "[run_tests] Sin tool_plugins/python: omitiendo pytest tool_plugins/python/tests/."
    fi
