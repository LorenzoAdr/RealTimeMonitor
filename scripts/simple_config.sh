...
_apply_simple_config() {
  case "${VARMON_RUN_MODE}" in
    code)
      ...
      ;;
    package)
      export VARMON_CONFIG="${VARMON_CONFIG:-$VARMON_INSTALL_DIR/varmon.conf}"
      export VARMON_PACKAGED_WEB_BIN="${VARMON_PACKAGED_WEB_BIN:-$VARMON_INSTALL_DIR/varmonitor-web}"
      export VARMON_SIDECAR_BIN="${VARMON_SIDECAR_BIN:-$VARMON_INSTALL_DIR/varmon_sidecar}"
